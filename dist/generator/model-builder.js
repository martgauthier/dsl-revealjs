...
const COMPONENT_BUILDERS = {
    ...
    FrameComponent: (ast) => {
        const components = ast.components.map((c) => {
 ...
 */
export function buildDiapo(diapoAst) {
    const slides = diapoAst.slides.map((abstractSlideAst) => {
...
function buildSlide(slideAst) {
    const components = slideAst.components.map((c) => {
        const builder = COMPONENT_BUILDERS[c.$type];
        if (!builder) {
            throw new Error(`Unknown component type: ${c.$type}`);
        }
    ...
    return new Slide(undefined, // transitionIn
    undefined, // transitionOut
    [], // steps (actions)
    components // components
    );
}
function buildNestedSlide(nestedSlideAst) {
    const subSlides = nestedSlideAst.subSlides.map((slideAst) => buildSlide(slideAst));
    return new NestedSlide(undefined, // transitionIn
    undefined, // transitionOut
    [], // steps (actions)
    subSlides // subSlides
    );
